...
async fn lobby_screen() -> Option<(std::net::SocketAddr, String)> {
    ...
    loop {
        ...
        // Backdrop block
        draw_rectangle(tx - 20.0, ty - th - 20.0, tw + 40.0, th + 34.0, Color::new(0.0, 0.0, 0.0, 0.52));
        draw_rectangle_lines(tx - 23.0, ty - th - 23.0, tw + 46.0, th + 40.0, 3.0, Color::new(0.58, 0.46, 0.0, 0.88));
        draw_rectangle_lines(tx - 20.75, ty - th - 20.75, tw + 41.5, th + 35.5, 1.5, Color::new(0.28, 0.22, 0.0, 0.60));
        ...
        let sw2 = sm.width;
        let sh2 = sm.height;
        draw_text(sub, cx - sw2*0.5 + 2.0, ty + 17.0 + sh2, ss, Color::new(0.0, 0.0, 0.0, 0.55));
        draw_text(sub, cx - sw2*0.5,        ty + 15.0 + sh2,       ss, Color::new(0.85, 0.85, 0.85, 1.0));
        ...
        if show_name_error || show_ip_error {
            ...
            let error_msg = if show_ip_error {"Please enter a valid IP!"} else { if name.trim().len() > 0 {"Name must be more than 2 characters!"} else {"Please enter your name to continue!"}};
